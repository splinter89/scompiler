...
<оператор> ::= <оператор_1>{,<оператор_1>}
<оператор_1> ::= [<объект>(=|+=|-=|*=|/=|%=)]<оператор_2>
<оператор_2> ::= <оператор_3>["||"<оператор_3>]
<оператор_3> ::= <оператор_4>[&&<оператор_4>]
<оператор_4> ::= <оператор_5>[(==|!=)<оператор_5>]
<оператор_5> ::= <оператор_6>[(<|<=|>|>=)<оператор_6>]
<оператор_6> ::= <оператор_7>[(+|-)<оператор_7>]
<оператор_7> ::= <оператор_8>[(*|/|%)<оператор_8>]
<оператор_8> ::= (<оператор_9>[(+|-|!|*)<оператор_9>]|(++|--|&)<объект>)
